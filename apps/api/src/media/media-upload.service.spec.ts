...
describe('MediaUploadService', () => {
  ...
  it('returns 503 when R2 is not configured', () => {
    ...
    expect(() => unconfigured.assertConfigured()).toThrow(ServiceUnavailableException);
  });

  it('rejects unsupported mime types', () => {
    expect(() => service.validateUpload(pngBuffer, 'image/svg+xml')).toThrow(BadRequestException);
  });

  it('rejects files above the size limit', () => {
    const oversized = Buffer.alloc(10 * 1024 * 1024 + 1);
    expect(() => service.validateUpload(oversized, 'image/png')).toThrow(BadRequestException);
  });

  it('rejects non-GLB mime for model3d kind', () => {
    expect(() => service.validateUpload(pngBuffer, 'image/png', 'model3d', 'model.glb')).toThrow(
      BadRequestException,
    );
  });
  ...
  it('rejects model3d uploads without .glb extension', () => {
    expect(() =>
      service.validateUpload(Buffer.from('glb'), 'model/gltf-binary', 'model3d', 'building.gltf'),
    ).toThrow(BadRequestException);
  });

  it('accepts valid model3d GLB uploads', () => {
  ...
  it('uploads and persists a company-scoped asset', async () => {
    ...
    expect(uploadObject).toHaveBeenCalledWith('companies/co_1/media_1.png', pngBuffer, 'image/png');
