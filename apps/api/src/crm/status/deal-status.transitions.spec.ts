...
describe("deal-status.transitions", () => {
  ...
  it("rejects transitions from terminal converted", () => {
    expect(
      isCrmStatusTransitionAllowed(
  ...
  it("maps statuses to buyer-facing labels", () => {
    expect(mapDealStatusToBuyerFacing(CrmDealStatus.new_request)).toBe(
      "request_sent",
    );
    expect(mapDealStatusToBuyerFacing(CrmDealStatus.contacted)).toBe(
      "in_contact",
    );
