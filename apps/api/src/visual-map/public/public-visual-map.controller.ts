...
export class PublicVisualMapController {
  ...
  listForProject(@Param('id') projectId: string): Promise<PublicVisualCanvasListResponse> {
    return this.visualMapService.listForProject(projectId);
  }

  @Public()
  @Get('districts/:id/visual-canvases')
  @ApiOperation({ summary: 'Published primary district visual canvas' })
  @ApiOkResponse({ description: 'Published visual navigation for a district' })
  @ApiNotFoundResponse({ description: 'District not found or not published' })
  listForDistrict(@Param('id') districtId: string): Promise<PublicVisualCanvasListResponse> {
    return this.visualMapService.listForDistrict(districtId);
  }
  ...
  listForBuilding(@Param('id') buildingId: string): Promise<PublicVisualCanvasListResponse> {
  ...
  listForFloor(@Param('id') floorId: string): Promise<PublicVisualCanvasListResponse> {
