...
import { assertTargetTypeMatchesContext, expectedTargetType } from './target-type.js';
...
describe('visual map context/target rules', () => {
  it('maps each context layer to the expected target type', () => {
    expect(expectedTargetType('project')).toBe('district');
    expect(expectedTargetType('district')).toBe('building');
  ...
  it('rejects wrong target type for a context', () => {
    expect(() => assertTargetTypeMatchesContext('project', 'floor')).toThrow(BadRequestException);
    expect(() => assertTargetTypeMatchesContext('building', 'apartment')).toThrow(
      BadRequestException,
    );
  });

  it('allows district or legacy building targets on project canvases', () => {
    expect(() => assertTargetTypeMatchesContext('project', 'district')).not.toThrow();
    expect(() => assertTargetTypeMatchesContext('project', 'building')).not.toThrow();
  });

  it('requires project contextId to equal projectId', () => {
    expect(() => assertProjectContextId('proj_1', 'proj_2')).toThrow(BadRequestException);
...
describe('resolveTargetStatus', () => {
  it('flags missing, unpublished, and ok targets', async () => {
    const { resolveTargetStatus, isPublicTargetOk } = await import('./target-status.js');

    expect(resolveTargetStatus(null)).toBe('missing');
    expect(resolveTargetStatus({ publicationStatus: PublicationStatus.draft })).toBe('unpublished');
    expect(resolveTargetStatus({ publicationStatus: PublicationStatus.published })).toBe('ok');
    expect(isPublicTargetOk({ publicationStatus: PublicationStatus.published })).toBe(true);
