...
import {
  ...
  UpsertReadinessCriterionScoreDto,
  UpsertReadinessCriterionScoresBatchDto,
...
export class AdminReadinessAssessmentsController {
  ...
  list(@Query() query: ListReadinessAssessmentsQueryDto): Promise<ReadinessAssessmentListResponse> {
  ...
  create(@Body() body: CreateReadinessAssessmentDto): Promise<ReadinessAssessmentDetail> {
    return this.assessments.create(body);
  }

  @Post('ensure-builders')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Ensure every builder company has an active company-level assessment',
  })
  @ApiOkResponse({ description: 'Number of assessments created for missing builders' })
  ensureBuilders(): Promise<{ createdCount: number }> {
  ...
  @Put(':id/criteria/:criterionId')
  @ApiOperation({ summary: 'Upsert KPI criterion score and recalculate category/overall' })
  @ApiOkResponse({ description: 'Updated assessment detail' })
  upsertCriterionScore(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
    @Param('criterionId') criterionId: string,
    @Body() body: UpsertReadinessCriterionScoreDto,
  ): Promise<ReadinessAssessmentDetail> {
    return this.assessments.upsertCriterionScore(id, criterionId, user.id, body);
  }

  @Put(':id/criteria')
  @ApiOperation({ summary: 'Batch upsert KPI criterion scores and recalculate totals' })
  @ApiOkResponse({ description: 'Updated assessment detail' })
  upsertCriterionScoresBatch(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
    @Body() body: UpsertReadinessCriterionScoresBatchDto,
  ): Promise<ReadinessAssessmentDetail> {
    return this.assessments.upsertCriterionScoresBatch(id, user.id, body);
  }
