...
describe('normalizeProjectRef', () => {
  ...
  it('decodes percent-encoded and double-encoded slugs', () => {
    ...
    expect(normalizeProjectRef(encodeURIComponent(encodeURIComponent('տեստ')))).toBe('տեստ');
    expect(normalizeProjectRef('Davinchi%2520_Clubhouse_view')).toBe('Davinchi _Clubhouse_view');
  });
});
