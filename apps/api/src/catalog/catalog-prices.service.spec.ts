...
const decimal = (value: string) => ({
  toString: () => value,
  lt: (other: { toString: () => string }) => Number(value) < Number(other.toString()),
  gt: (other: { toString: () => string }) => Number(value) > Number(other.toString()),
});
...
describe('CatalogPricesService', () => {
  ...
  it('throws NotFound for missing or unpublished project', async () => {
    ...
    await expect(service.getProjectPrices('missing')).rejects.toBeInstanceOf(NotFoundException);
  });
