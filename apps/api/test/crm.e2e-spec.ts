...
describe("CRM lead intake + Constructor CRM (e2e)", () => {
  ...
  it("buyer creates request → deal appears in builder CRM", async () => {
    ...
    expect(
      list.body.data.some((d: { id: string }) => d.id === create.body.dealId),
    ).toBe(true);
  });
  ...
  it("repeat buyer request deduplicates into same open deal", async () => {
    ...
    expect(
      detail.body.activities.some(
        (a: { title: string }) =>
          a.title === "Additional intake request attached",
      ),
    ).toBe(true);
  });
    ...
    expect(
      detail.body.requests.some(
        (r: { scanEventId: string | null; source: string }) =>
          r.scanEventId === scanEventId &&
          r.source === "builder_buyer_qr_scan",
      ),
    ).toBe(true);
  });
  ...
  it("buyer sees own requests without CRM notes; other buyer isolated", async () => {
    ...
    expect(
      JSON.stringify(mine.body).includes("Internal note — must not leak"),
    ).toBe(false);
    ...
    expect(
      otherList.body.data.some(
        (r: { requestId: string }) => r.requestId === create.body.requestId,
      ),
    ).toBe(false);
  });
  ...
  it("notes and activities work; cross-company deal is 404", async () => {
    ...
    const activity = await request(app.getHttpServer())
      .post(
        `${API_V1_PREFIX}/portal/crm/deals/${created.body.dealId}/activities`,
      )
      .set("Cookie", builder.cookie)
      .set(CSRF_HEADER_NAME, builder.csrf)
      .set("Origin", "http://localhost:3000")
