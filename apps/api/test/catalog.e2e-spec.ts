...
describe('Catalog public endpoints (e2e)', () => {
  ...
  it('lists only published projects and supports pagination meta', async () => {
    ...
    expect(
      response.body.data.some((project: { id: string }) => project.id === publishedProjectId),
    ).toBe(true);
    expect(
      response.body.data.some((project: { id: string }) => project.id === draftProjectId),
    ).toBe(false);
  });
  ...
  it('lists active builders with published project counts', async () => {
    ...
    const builder = response.body.find((item: { id: string }) => item.id === builderId);
  ...
  it('returns builder profile with published projects', async () => {
    ...
    expect(
      response.body.projects.some((project: { id: string }) => project.id === publishedProjectId),
    ).toBe(true);
  });
