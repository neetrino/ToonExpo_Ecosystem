...
import { isCsrfForbiddenMessage } from './errors';
...
describe('csrf helpers', () => {
  ...
  it('detects CSRF forbidden messages vs permission denials', () => {
    expect(isCsrfForbiddenMessage('CSRF token mismatch')).toBe(true);
    expect(isCsrfForbiddenMessage('Invalid CSRF token')).toBe(true);
    expect(isCsrfForbiddenMessage('Company admin access required')).toBe(false);
    expect(isCsrfForbiddenMessage(undefined)).toBe(false);
  });
});
