...
export const ProjectBuildings = async ({ projectId, buildings }: ProjectBuildingsProps) => {
  ...
  if (buildings.length === 0) {
    return <p className="text-sm text-ink-secondary">{t('project.noBuildings')}</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      {buildings.map((building) => (
        <section
          key={building.id}
          className="overflow-hidden rounded-md border border-border/80 bg-surface-elevated shadow-xs"
        >
          <div className="grid gap-0 sm:grid-cols-[12rem_1fr]">
            ...
            >
              {building.cover ? (
                ...
                <div className="flex size-full min-h-32 items-center justify-center bg-surface text-xs text-ink-muted">
                  {building.name}
                </div>
              )}
            </Link>
