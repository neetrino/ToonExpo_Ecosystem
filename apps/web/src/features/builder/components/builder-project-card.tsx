...
type BuilderProjectStatProps = {
  icon: LucideIcon;
  label: string;
  value: number;
};

const BuilderProjectStat = ({ icon: Icon, label, value }: BuilderProjectStatProps) => (
  <span className="flex min-w-0 items-center gap-2.5">
    ...
    <span className="flex min-w-0 flex-col leading-tight">
      ...
      <span className="text-base font-semibold tabular-nums tracking-tight text-ink">{value}</span>
    </span>
  </span>
);

/**
 * KPI-style project card — company logo, title, cover, inventory footer (matches admin).
 */
