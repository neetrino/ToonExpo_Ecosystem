...
const AssessmentView = ({ assessment }: { assessment: PortalReadinessAssessmentItem }) => {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-base font-semibold text-ink">{t('categoriesTitle')}</h2>
      ...
      {assessment.requiredActions.length > 0 ? (
        <section className="flex flex-col gap-3">
          <h2 className="text-base font-semibold text-ink">{t('requiredActionsTitle')}</h2>
      ...
      {assessment.recommendations.length > 0 ? (
        <section className="flex flex-col gap-3">
          <h2 className="text-base font-semibold text-ink">{t('recommendationsTitle')}</h2>
          <ul className="flex flex-col gap-2">
            {assessment.recommendations.map((rec) => (
              <li key={rec.id} className="rounded-sm border border-border p-4">
                <p className="font-medium text-ink">{rec.title}</p>
                <p className="mt-1 text-sm text-ink-secondary">{rec.description}</p>
              </li>
            ))}
          </ul>
        </section>
