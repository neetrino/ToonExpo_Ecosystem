...
const isPathInSection = (pathname: string, item: BuilderNavItem): boolean => {
...
const initialOpenSections = (pathname: string): Record<string, boolean> => {
  ...
  for (const item of BUILDER_SECTION_NAV_ITEMS) {
...
export const BuilderNav = ({ companyName }: BuilderNavProps) => {
  ...
  const isItemActive = (href: string): boolean => {
    ...
    return !BUILDER_ALL_NAV_ITEMS.some(
      (item) =>
        item.href !== href &&
  ...
  const toggleSection = (key: BuilderNavItemKey): void => {
    setOpenSections((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const openSectionsKey = BUILDER_SECTION_NAV_ITEMS.map(
    (item) => `${item.key}:${openSections[item.key] ? '1' : '0'}`,
  ).join('|');
  const measureKey = `${pathname}|${railCollapsed ? '1' : '0'}|${openSectionsKey}`;

  const renderNavItem = (item: BuilderNavItem) => {
    ...
    const subnavId = `builder-${item.key}-subnav`;
  ...
  return (
    <nav aria-label={t('label')} className="flex h-full min-h-0 flex-col gap-1">
      ...
      <PortalNavRailGroup
        ...
        gapClassName="gap-1"
      >
        {BUILDER_PRIMARY_NAV_ITEMS.map(renderNavItem)}
      </PortalNavRailGroup>
      ...
      <PortalNavRailGroup
        ...
        gapClassName="gap-1"
      >
        {renderNavItem(BUILDER_SETTINGS_NAV_ITEM)}
      </PortalNavRailGroup>
    </nav>
  );
};
