...
const BUILDER_HUB_HREF = '/builder';

type BuilderMobileStackProps = {
  name: string;
  email: string;
...
/**
 * Mobile builder stack: hub always shows first; sub-pages slide over it.
 */
export const BuilderMobileStack = ({
  name,
  email,
  companyName,
  children,
}: BuilderMobileStackProps) => {
  const pathname = usePathname();
  const router = useRouter();
  const isHub = pathname === BUILDER_HUB_HREF;
  const [mounted, setMounted] = useState(false);
  const [sheetOpen, setSheetOpen] = useState(false);
  const [anim, setAnim] = useState<PanelAnim>('in');
  const exitingRef = useRef(false);
  const sawHubRef = useRef(pathname === BUILDER_HUB_HREF);
