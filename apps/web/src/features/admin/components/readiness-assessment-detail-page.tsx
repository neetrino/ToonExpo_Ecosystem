  ...
  if (query.isError || !query.data) {
    return (
      <div className="flex flex-col gap-3">
        ...
        <Link href="/admin/readiness" className="text-sm font-medium text-brand hover:underline">
          {t('back')}
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-page-title text-ink">{companyName}</h1>
        ...
        <div className="flex flex-wrap gap-4 text-sm text-ink-secondary">
          ...
          <span>
            {t('lastEvaluated')}: {formatReadinessDate(assessment.lastEvaluatedAt, locale)}
          </span>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-4">
        <h2 className="text-base font-semibold text-ink">{t('categories.title')}</h2>
        <div className="flex flex-col gap-4">
          {assessment.scores.map((score) => (
            <ReadinessCategoryScoreRow key={score.id} assessmentId={assessment.id} score={score} />
          ))}
        </div>
      </section>
