...
| Pre-commit hooks   | Husky + lint-staged                     | Confirmed | ESLint `--fix` and Prettier on staged files only; installed via root `prepare` on `pnpm install`.                                                                                        |
| Dependency updates | Dependabot                              | Confirmed | Weekly npm (pnpm workspace) and GitHub Actions updates; minor/patch grouped to reduce PR noise.                                                                                          |

## Developer Tooling

| Tool        | Purpose                                                                                                  |
| ----------- | -------------------------------------------------------------------------------------------------------- |
| Husky       | Git hooks (`prepare` script runs on `pnpm install`)                                                      |
| lint-staged | Pre-commit: ESLint `--fix` on staged `*.{ts,tsx}`; Prettier `--write` on staged `*.{ts,tsx,json,md,css}` |
| Dependabot  | Weekly dependency PRs for npm workspace and GitHub Actions (minor/patch grouped)                         |
| commitlint  | Conventional Commits (`.commitlintrc.json`; enforce via CI or optional commit-msg hook)                  |
...
| Product data lifecycle | PostgreSQL only; repository scripts never populate or reset product records                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Cache/queues           | **Next.js Data Cache** for anonymous public SSR/RSC GETs. Catalog **lists** TTL 30 min; **project detail** / visual-map / geo-map TTL **0** (immediate after Admin save). Tag purge on publish and on published project content updates via `POST /api/revalidate` when `WEB_REVALIDATE_*` is set. Cached catalog HTML is always anonymous; logged-in `visible_after_login` prices are merged client-side via the private overlay (`GET /catalog/projects/:id/prices`, `GET /catalog/projects/prices?ids=…` — session-only, `no-store`). No Redis application/data cache. Redis remains optional for distributed rate limits only (Upstash). |
