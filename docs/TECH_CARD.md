...
| City 3D map        | MapLibre GL JS + Three.js               | Confirmed | Platform-admin GLB placements linked to catalog buildings; public homepage MAP VIEW is read-only (pins + models). Persistence via NestJS + R2; no product API in Next.js.                |
