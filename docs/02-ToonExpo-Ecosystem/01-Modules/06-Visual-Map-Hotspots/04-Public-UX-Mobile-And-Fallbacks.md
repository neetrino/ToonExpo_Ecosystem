...
Public apartment detail shows the same sales status as a chip: on the gallery (top-right) on mobile, and opposite the catalog path breadcrumb on desktop. Platform admins can change it there via a dropdown; everyone else sees a read-only status chip. The write is manual inventory SoT and syncs the linked CRM deal when one exists.
