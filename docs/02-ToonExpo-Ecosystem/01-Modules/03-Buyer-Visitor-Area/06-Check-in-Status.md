# Check-in Status

## Purpose

Buyer can understand whether they have checked in to an event.

## Check-in Source

Check-in is created by Exhibition Map & Check-in module when entrance staff scans buyer QR.

## Buyer View

Buyer can see:

- current event check-in status;
- checked-in time if checked in;
- event name;
- My QR access.

## Multiple Events

Buyer can have many check-in records over time.

v1 can show current/latest event first.

## No Paid Ticket Wallet

Check-in status is not a paid ticket wallet.

No payment/ticket validation exists in v1.

## Entrance Denial

If QR is invalid/blocked, entrance staff sees result.

Buyer-facing blocked/invalid messaging should be handled carefully by support/admin flow.

