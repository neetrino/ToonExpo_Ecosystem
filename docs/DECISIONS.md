...
- **Project price on request (v1):** builder `company_admin` or Admin catalog may enable `price_on_request_enabled` on a project (switcher next to project QR). Public catalog hides all apartment prices in that project. Toggling cascades to existing buildings; new buildings inherit the project flag.
